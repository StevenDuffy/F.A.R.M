...
namespace FarmControl
{
    public class ActivityLogger
    {
        ...
        public void AddFertiliserStockFailed(short amount)
        {
            using (activityRecorder = new StreamWriter(activityLogFile, true))
            {
                activityRecorder.WriteLine("Attempt to add {0}kgs to ertiliser stock failed: {1}", amount, DateTime.Now);
            }
        }

        public void AddFertiliserStockSucceeded(short amount)
        {
            using (activityRecorder = new StreamWriter(activityLogFile, true))
            {
                activityRecorder.WriteLine("Attempt to add {0}kgs to fertiliser stock succeeded: {1}", amount, DateTime.Now);
            }
        }

        public void RemoveFertiliserStockFailed(short amount)
        {
            using (activityRecorder = new StreamWriter(activityLogFile, true))
            {
                activityRecorder.WriteLine("Attempt to remove {0}kgs from Fertiliser stock failed: {1}", amount, DateTime.Now);
            }
        }

        public void RemoveFertiliserStockSucceeded(short amount)
        {
            using (activityRecorder = new StreamWriter(activityLogFile, true))
            {
                activityRecorder.WriteLine("Attempt to remove {0}kgs from Fertiliser stock succeeded: {1}", amount, DateTime.Now);
            }
        }
